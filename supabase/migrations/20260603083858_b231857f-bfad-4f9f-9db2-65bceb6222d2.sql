...
-- 1. Customer accounts table (groups)
...
-- Case-insensitive uniqueness on account name
CREATE UNIQUE INDEX IF NOT EXISTS customer_accounts_name_lower_uniq
  ON public.customer_accounts (lower(name));

-- 2. GRANTs (auth users can read+create; admins can update/delete)
GRANT SELECT, INSERT ON public.customer_accounts TO authenticated;
GRANT UPDATE, DELETE ON public.customer_accounts TO authenticated; -- gated by RLS
GRANT ALL ON public.customer_accounts TO service_role;

-- 3. RLS
ALTER TABLE public.customer_accounts ENABLE ROW LEVEL SECURITY;

CREATE POLICY cust_accounts_select_auth
  ON public.customer_accounts FOR SELECT
  TO authenticated USING (true);

CREATE POLICY cust_accounts_insert_auth
  ON public.customer_accounts FOR INSERT
  TO authenticated WITH CHECK (true);

CREATE POLICY cust_accounts_update_admin
  ON public.customer_accounts FOR UPDATE
  TO authenticated USING (is_admin_or_higher());

CREATE POLICY cust_accounts_delete_admin
...
BEGIN
  ...
  INSERT INTO public.customers (
    ...
    p_lead_metadata, p_account_id
  ) RETURNING id INTO v_id;
  RETURN v_id;
END;
$$;

-- update_customer_secure: add p_account_id
DROP FUNCTION IF EXISTS public.update_customer_secure(
  uuid, text, text, text, text, text, text, text, text, text, text, text, text, text, text, date, text, jsonb, jsonb, text, jsonb, text, text, jsonb, jsonb, jsonb, text, text, text, text, text, text, text, text, jsonb, text, numeric, numeric, text, numeric, integer, numeric, text, numeric, numeric, integer, text, numeric, text, numeric, integer, numeric, jsonb, numeric
);
