...
-- Create secure email masking function
...
-- Drop existing permissive policies that pose security risks
...
-- Create ultra-restrictive default policy that blocks most direct access
CREATE POLICY "Block_direct_comprehensive_users_access" 
ON public.comprehensive_users 
FOR SELECT 
USING (false);

-- Allow only Super-Admin emergency access for direct table access
...
-- Allow HR and Super-Admin to update records (with audit logging via triggers)
...
-- Only Super-Admin can delete (emergency situations only)
