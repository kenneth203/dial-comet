...
CREATE OR REPLACE FUNCTION public.get_holiday_admin_overview()
 RETURNS TABLE(system_user_id uuid, auth_user_id uuid, name text, email text, role text, department text, status text, base_annual numeric, carried_over numeric, bank_holidays numeric, christmas_closure numeric, sick_days numeric, personal_days numeric, available_for_booking numeric, annual_booked numeric, annual_remaining numeric, sick_remaining numeric, personal_remaining numeric, personal_taken numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DELETE FROM holiday_requests WHERE id = '28d7ef3c-9f2c-49b5-a3f9-4b69b4c10420' AND status = 'pending';