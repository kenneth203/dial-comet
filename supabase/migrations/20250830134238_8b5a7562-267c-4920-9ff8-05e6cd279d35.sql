-- Check for function conflicts and fix the create_holiday_request_admin function
DROP FUNCTION IF EXISTS public.create_holiday_request_admin(uuid,date,date,absence_type,text);

-- Recreate the function with proper signature to avoid conflicts
...
BEGIN
  ...
  IF target_user_id IS NULL THEN
    RAISE EXCEPTION 'Target user not found or not active. System user ID: %', target_system_user_id;
  END IF;
