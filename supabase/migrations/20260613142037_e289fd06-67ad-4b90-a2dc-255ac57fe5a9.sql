...
-- Add is_private flag to chat_rooms
ALTER TABLE public.chat_rooms ADD COLUMN IF NOT EXISTS is_private boolean NOT NULL DEFAULT false;

-- RPC: create a private channel with explicit members (Admin/Super-Admin only)
...
-- RPC: replace channel members (keeps the creator)
...
-- RPC: list members of a channel (auth callers who are members, or admins)
...
BEGIN
  ...
  IF NOT public.is_admin_strictly() AND NOT EXISTS (
...
GRANT EXECUTE ON FUNCTION public.create_private_channel(text, uuid[]) TO authenticated;
