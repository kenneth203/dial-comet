-- Fix critical security and data integrity issues (corrected syntax)
...
-- 4. Add constraints for data validation (handle existing constraint)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE table_name = 'holiday_entitlements' 
    AND constraint_name = 'check_positive_days'
  ) THEN
