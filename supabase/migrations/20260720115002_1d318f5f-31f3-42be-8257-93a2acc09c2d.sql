...
BEGIN
  ...
  IF v_inst.template_id IS NOT NULL THEN
    -- Fan out completion to sibling rows for other operators on the same
    -- template/date/occurrence. Copy contact_names across so the
    -- enforce_checklist_contact_names trigger does not block the fan-out
    -- when the template requires a minimum number of contacts.
    UPDATE public.checklist_instances SET
      status = 'completed',
      completed_at = COALESCE(completed_at, now()),
      completed_by = COALESCE(completed_by, v_uid),
      completion_notes = COALESCE(p_notes, completion_notes),
      contact_names = CASE
        WHEN v_require AND COALESCE(cardinality(contact_names), 0) < v_min
          THEN v_inst.contact_names
        ELSE contact_names
      END,
      is_overdue = false,
      updated_at = now()
