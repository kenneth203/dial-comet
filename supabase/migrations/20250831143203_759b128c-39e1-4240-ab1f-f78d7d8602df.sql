...
DROP FUNCTION IF EXISTS public.get_system_user_holiday_breakdown(uuid);

-- Create function to sync system_users holiday data to holiday_entitlements
...
-- Create trigger to automatically sync system_users to holiday_entitlements
DROP TRIGGER IF EXISTS sync_system_user_holiday_data ON public.system_users;
CREATE TRIGGER sync_system_user_holiday_data
  AFTER INSERT OR UPDATE OF annual_leave_days, sick_leave_days, personal_days, carried_over_days
  ON public.system_users
  FOR EACH ROW
  EXECUTE FUNCTION sync_system_user_to_holiday_entitlements();

-- Create function to get holiday breakdown from system_users
