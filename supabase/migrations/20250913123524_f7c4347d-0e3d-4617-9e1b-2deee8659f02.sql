-- =============================================
-- SECURE PERMISSIONS MATRIX ACCESS CONTROL - CLEAN FIX
-- =============================================

-- Step 1: Drop any existing functions to avoid conflicts
...
-- Step 2: Enhance the secure permissions access function  
...
BEGIN
    ...
    -- Log the access attempt for security auditing (if audit table exists)
    BEGIN
...
CREATE FUNCTION public.update_permission_grant_secure(
...
BEGIN
    ...
    -- Log the permission change (if audit table exists)
    BEGIN
        INSERT INTO public.system_users_audit_log (
            accessed_by, 
            employee_user_id, 
...
-- Step 4: Create the monitoring function
CREATE FUNCTION public.detect_unauthorized_permissions_access()
RETURNS TABLE(
    user_id UUID,
    user_name TEXT,
...
BEGIN
    ...
    -- Return analysis if audit table exists, otherwise empty result
    BEGIN
        RETURN QUERY
        SELECT 
...
-- Step 5: Ensure the view is properly secured
