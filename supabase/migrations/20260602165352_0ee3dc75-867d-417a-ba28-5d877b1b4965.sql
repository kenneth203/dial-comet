CREATE OR REPLACE FUNCTION public.get_holiday_admin_overview(p_year integer DEFAULT (EXTRACT(year FROM CURRENT_DATE))::integer)
 RETURNS TABLE(user_id uuid, user_name text, user_role text, annual_leave_entitlement numeric, annual_leave_used numeric, sick_leave_used numeric, personal_days_used numeric, pending_requests bigint)
 LANGUAGE sql
