...
-- 1. Create a session variable to bypass ownership checks for trusted admin functions
-- This allows admin functions to work around data inconsistencies
...
-- 5. Update decline function to also use bypass for consistency
...
BEGIN
  ...
  -- Reconcile system_user_id if needed (same as approve)
  IF req.system_user_id IS NULL OR NOT EXISTS (
    SELECT 1 FROM public.system_users su 
    WHERE su.id = req.system_user_id 
    AND su.user_id = req.user_id
