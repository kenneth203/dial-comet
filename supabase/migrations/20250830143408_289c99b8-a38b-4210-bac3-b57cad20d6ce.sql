-- Check the constraint and temporarily remove it to fix the duplicate user_id issue
...
-- Step 2: Update user_ids to unique values for each user
DO $$
DECLARE
...
BEGIN
    ...
    -- Update comprehensive_users to include all users
...
-- Step 3: Clear existing holiday requests since we can't determine which belong to whom
-- All current requests were associated with Kenneth's ID, so we'll delete them
-- Users will need to re-create their holiday requests
DELETE FROM holiday_requests WHERE user_id = '34fd1b64-9190-4e31-8c20-b20705053fc1';

-- Step 4: Create holiday entitlements for each user for the current year
