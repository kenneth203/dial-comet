-- CRITICAL SECURITY FIXES: Comprehensive security hardening
...
-- Fix 3: Secure the encrypt-financial-data edge function by ensuring proper key handling
-- (This will be handled in the edge function code update)

-- Fix 4: Add audit trigger for sensitive table access
...
DROP TRIGGER IF EXISTS audit_system_users_access ON public.system_users;
CREATE TRIGGER audit_system_users_access
    BEFORE SELECT OR INSERT OR UPDATE OR DELETE ON public.system_users
    FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_table_access();

DROP TRIGGER IF EXISTS audit_employee_financial_data_access ON public.employee_financial_data;
CREATE TRIGGER audit_employee_financial_data_access
    BEFORE SELECT OR INSERT OR UPDATE OR DELETE ON public.employee_financial_data
    FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_table_access();

-- Fix 5: Create secure database reset function with enhanced validation
...
-- Fix 6: Revoke execute from anon on the new reset function
REVOKE EXECUTE ON FUNCTION public.perform_database_reset FROM anon;
GRANT EXECUTE ON FUNCTION public.perform_database_reset TO authenticated;

-- Security hardening complete
SELECT 'CRITICAL_SECURITY_FIXES_APPLIED: RLS tightened, sensitive RPCs secured, audit logging enhanced' as status;