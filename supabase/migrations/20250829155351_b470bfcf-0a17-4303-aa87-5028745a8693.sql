-- Fix holiday entitlements calculation to properly read from user setup
-- The issue is that we need to combine public_holidays and christmas_closure_days into a single "Bank Holiday/Closures" deduction
...
BEGIN
  ...
  -- Calculate total Bank Holiday/Closures deduction (combines public holidays + christmas closure)
  total_bank_holidays_deduction := 
    COALESCE(user_record.public_holidays, 10.0) + 
    COALESCE(user_record.christmas_closure_days, 5.0);
  
  -- Calculate annual leave allowed (base + carried over - total bank holidays/closures)
  ...
  SELECT 
    ...
    GREATEST(calculated_annual_leave_allowed - annual_used, 0) as annual_leave_remaining,
    -- Sick Leave
    ...
    -- Public Holidays (read-only, showing combined total)
