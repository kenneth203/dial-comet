...
-- Create highly restrictive policies that block direct access
CREATE POLICY "Block_direct_system_users_access" ON public.system_users
    FOR SELECT USING (false);

CREATE POLICY "Block_direct_system_users_insert" ON public.system_users
    FOR INSERT WITH CHECK (false);

CREATE POLICY "Block_direct_system_users_update" ON public.system_users
    FOR UPDATE USING (false);

CREATE POLICY "Block_direct_system_users_delete" ON public.system_users
    FOR DELETE USING (false);

-- Only HR and Super-Admin can perform emergency operations
CREATE POLICY "HR_SuperAdmin_emergency_system_users_access" ON public.system_users
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE user_id = auth.uid() 
...
-- Step 7: Securely clear financial data from system_users after migration
-- Clear financial data from system_users as it's now in encrypted employee_financial_data
...
-- Step 8: Create enhanced mask functions
