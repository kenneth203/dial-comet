...
-- 1) Ensure updated_at is maintained automatically on update
...
-- 2) Deduplicate system_users by user_id (keep the most recent row)
WITH ranked AS (
  SELECT
    id,
    user_id,
...
-- 4) Recreate the RPC to always read the latest system_users row for a given auth user
...
BEGIN
  -- Always select the most recent row for this auth user_id
  ...
  -- Bank Holiday/Closures = bank holidays + christmas closure
  total_bank_holidays_deduction :=
    COALESCE(user_record.public_holidays, 10.0) +
    COALESCE(user_record.christmas_closure_days, 5.0);

  -- Allowed annual leave = base + carried over - bank holidays - xmas closure
