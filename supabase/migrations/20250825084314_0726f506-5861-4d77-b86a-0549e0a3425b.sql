-- Enhanced security for staff_details table
...
CREATE OR REPLACE FUNCTION public.get_staff_contact_info(
    target_user_id UUID,
    access_reason TEXT DEFAULT NULL
)
RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION public.get_staff_personal_details(
    target_user_id UUID,
    access_reason TEXT
)
RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION public.get_staff_emergency_contacts(
    target_user_id UUID,
    access_reason TEXT
)
RETURNS TABLE(
...
-- Create new restrictive policies - no direct access, only through functions
CREATE POLICY "No_direct_staff_details_access" ON public.staff_details
    FOR ALL USING (false);
