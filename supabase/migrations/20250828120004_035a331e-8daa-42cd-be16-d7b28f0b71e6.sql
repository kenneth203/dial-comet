...
-- The user_id should be nullable for staff-only records or reference a different approach

-- First, let's check and fix the foreign key constraint
-- Drop the existing foreign key constraint if it exists
ALTER TABLE public.staff_details DROP CONSTRAINT IF EXISTS fk_staff_details_user;

-- Since staff records can exist without corresponding auth users,
-- we'll make user_id nullable and not enforce a foreign key constraint
-- for staff-only records. For system users, we'll handle the relationship
-- at the application level.

-- Update the create_staff_member_secure function to handle this properly
...
BEGIN
    ...
    -- For staff-only records, we don't need a user_id
    ...
    IF create_system_user THEN
        ...
        created_user := true;
    ELSE
        -- For staff-only records, leave user_id as NULL
    ...
    INSERT INTO public.staff_details (
        ...
        new_auth_user_id, -- This will be NULL for staff-only records
