CREATE OR REPLACE FUNCTION public.append_todo_note(
  p_task_id uuid,
  p_body text,
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.todos WHERE id = p_task_id) THEN
    RAISE EXCEPTION 'Task not found.';
  END IF;

  v_author_name := btrim(COALESCE(NULLIF(p_author_name, ''), 'User'));
  v_author_name := regexp_replace(v_author_name, '[\|\[\]\r\n]+', ' ', 'g');
  v_author_name := left(v_author_name, 120);

  v_entry := format('[[note:%s|%s|%s]]%s%s', v_uid, v_author_name, to_char(clock_timestamp() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'), E'\n', v_body);

  UPDATE public.todos
  SET notes = CASE
        WHEN btrim(COALESCE(notes, '')) = '' THEN v_entry
        ELSE notes || E'\n\n' || v_entry
      END,
      updated_at = now()
  WHERE id = p_task_id
...
REVOKE ALL ON FUNCTION public.append_todo_note(uuid, text, text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.update_todo_note_body(uuid, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.append_todo_note(uuid, text, text) TO authenticated, service_role;
