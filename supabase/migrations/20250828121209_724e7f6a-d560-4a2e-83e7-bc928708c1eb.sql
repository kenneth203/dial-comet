-- Fix the check constraint and test staff creation completely

-- Update the check constraint to allow the new data type value
...
CHECK (data_type = ANY (ARRAY[
    'FULL_STAFF_ACCESS'::text, 
    'BASIC_STAFF_INFO'::text, 
    'CONTACT_INFO'::text, 
    'PERSONAL_DETAILS'::text, 
    'EMERGENCY_CONTACTS'::text, 
    'STAFF_DIRECTORY_ACCESS'::text, 
    'INDIVIDUAL_STAFF_ACCESS'::text, 
    'STAFF_ONLY_RECORD_CREATION'::text,
    'basic_staff_info'::text, 
    'contact_info'::text, 
    'personal_details'::text, 
    'emergency_contacts'::text, 
    ...
    'bulk_access'::text, 
    'directory_access'::text
]));

-- Let's also simplify the function to use standard data types to avoid future issues
...
SELECT * FROM public.create_staff_member_secure(
    '{"email": "test.staff@example.com", "department": "Test Dept", "position": "Test Role", "phone_number": "07700900000"}'::jsonb,
    false,
    NULL,
    NULL
);
...
DELETE FROM public.staff_details WHERE email = 'test.staff@example.com';