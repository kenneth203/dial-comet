...
import { CLProposalPackagesTab } from "./proposal/CLProposalPackagesTab";
...
interface CLPackageProposalDialogProps {
  open: boolean;
...
export function CLPackageProposalDialog({ open, onOpenChange, initialData, onPackageSelected }: CLPackageProposalDialogProps) {
  const { packages } = usePackages();
  const clPackages = packages.filter(pkg => pkg.services.includes("CL"));
  ...
  const selectedPackage = clPackages.find(p => p.id === selectedPackageId);
  ...
  const renderPackagesWithForm = () => (
    <div className="space-y-6">
      <CLProposalPackagesTab
        clPackages={clPackages}
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3 text-sm text-muted-foreground">
            ...
            <div className="flex items-start space-x-2">
              ...
              />
              <Label htmlFor="cl-terms" className="text-sm leading-tight cursor-pointer">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="clFirstName">First Name *</Label>
                <Input id="clFirstName" value={formData.firstName} onChange={(e) => setFormData(f => ({ ...f, firstName: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="clLastName">Last Name *</Label>
                <Input id="clLastName" value={formData.lastName} onChange={(e) => setFormData(f => ({ ...f, lastName: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="clEmail">Email Address *</Label>
                <Input id="clEmail" type="email" value={formData.email} onChange={(e) => setFormData(f => ({ ...f, email: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="clTelephone">Telephone *</Label>
                <Input id="clTelephone" value={formData.telephone} onChange={(e) => setFormData(f => ({ ...f, telephone: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="clCompanyName">Company Name</Label>
                <Input id="clCompanyName" value={formData.companyName} onChange={(e) => setFormData(f => ({ ...f, companyName: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="clHeardAboutUs">How did you hear about us? *</Label>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-between">
              <DialogTitle>Call Answering Service (Clinic) Proposal Builder</DialogTitle>
