...
interface CBProposalPackagesTabProps {
  cbPackages: Package[];
  selectedPackageId: string | null;
  onSelectPackage: (id: string) => void;
}

export function CBProposalPackagesTab({ cbPackages, selectedPackageId, onSelectPackage }: CBProposalPackagesTabProps) {
    ...
    return (
      ...
      >
        <CardContent className="p-4 text-center space-y-3">
          <h3 className="text-sm font-bold text-primary">{pkg.name} (CB)</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-1">
        ...
        />
        <h2 className="text-xl font-bold text-foreground">Our Call Answering Services (Bookings)</h2>
        <p className="text-sm text-muted-foreground">Our Packages</p>
      </div>

      {cbPackages.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          <p className="font-medium">No Bookings packages available</p>
          <p className="text-sm mt-1">Add packages with the "CB" service on the Packages & Pricing page first.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {cbPackages.map(renderPackageCard)}
        </div>
      )}
    </div>
  );
