...
interface CLProposalPackagesTabProps {
  clPackages: Package[];
  selectedPackageId: string | null;
  onSelectPackage: (id: string) => void;
}

export function CLProposalPackagesTab({ clPackages, selectedPackageId, onSelectPackage }: CLProposalPackagesTabProps) {
    ...
    return (
      ...
      >
        <CardContent className="p-4 text-center space-y-3">
          <h3 className="text-sm font-bold text-primary">{pkg.name} (CL)</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-1">
        ...
        />
        <h2 className="text-xl font-bold text-foreground">Our Call Answering Services (Clinic)</h2>
        <p className="text-sm text-muted-foreground">Our Packages</p>
      </div>

      {clPackages.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          <p className="font-medium">No Clinic packages available</p>
          <p className="text-sm mt-1">Add packages with the "CL" service on the Packages & Pricing page first.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {clPackages.map(renderPackageCard)}
        </div>
      )}
    </div>
  );
