...
import { AIProposalPackagesTab } from "./proposal/AIProposalPackagesTab";
...
interface AIPackageProposalDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialData?: ProposalInitialData;
  onPackageSelected?: (pkg: { name: string; aiSetupFee: number; aiMonthlyFee: number; aiCallsAllocated: number }, proposal: ProposalRecord) => void;
}

export function AIPackageProposalDialog({ open, onOpenChange, initialData, onPackageSelected }: AIPackageProposalDialogProps) {
  const { packages } = usePackages();
  const aiPackages = packages.filter(pkg => pkg.services.includes("AI"));
  ...
  const selectedPackage = aiPackages.find(p => p.id === selectedPackageId);
  ...
  const handleFinalSubmit = () => {
    if (selectedPackage && onPackageSelected) {
      const proposalRecord: ProposalRecord = {
        ...
        packagePrice: selectedPackage.aiMonthlyFee,
        invoiceNumber,
      ...
      onPackageSelected({
        ...
        aiSetupFee: selectedPackage.aiSetupFee,
        aiMonthlyFee: selectedPackage.aiMonthlyFee,
        aiCallsAllocated: selectedPackage.aiCallsAllocated,
      }, proposalRecord);
    }
    toast({
      title: "Proposal Submitted",
      description: `${formData.firstName} ${formData.lastName} selected the "${selectedPackage?.name}" AI package. Proposal, agreement and invoice have been sent.`,
    });
    onOpenChange(false);
    resetForm();
  };
  ...
  const invoicePackage = selectedPackage ? { ...selectedPackage, price: selectedPackage.aiMonthlyFee } : undefined;
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3 text-sm text-muted-foreground">
            ...
            <div className="flex items-start space-x-2">
              ...
              />
              <Label htmlFor="ai-terms" className="text-sm leading-tight cursor-pointer">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="aiFirstName">First Name *</Label>
                <Input id="aiFirstName" value={formData.firstName} onChange={(e) => setFormData(f => ({ ...f, firstName: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="aiLastName">Last Name *</Label>
                <Input id="aiLastName" value={formData.lastName} onChange={(e) => setFormData(f => ({ ...f, lastName: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="aiEmail">Email Address *</Label>
                <Input id="aiEmail" type="email" value={formData.email} onChange={(e) => setFormData(f => ({ ...f, email: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="aiTelephone">Telephone *</Label>
                <Input id="aiTelephone" value={formData.telephone} onChange={(e) => setFormData(f => ({ ...f, telephone: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="aiCompanyName">Company Name</Label>
                <Input id="aiCompanyName" value={formData.companyName} onChange={(e) => setFormData(f => ({ ...f, companyName: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="aiHeardAboutUs">How did you hear about us? *</Label>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-between">
              <DialogTitle>AI Call Handling Service Proposal Builder</DialogTitle>
