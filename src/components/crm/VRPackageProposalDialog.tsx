...
import { VRProposalPackagesTab } from "./proposal/VRProposalPackagesTab";
...
interface VRPackageProposalDialogProps {
  open: boolean;
...
export function VRPackageProposalDialog({ open, onOpenChange, initialData, onPackageSelected }: VRPackageProposalDialogProps) {
  const { packages } = usePackages();
  const vrPackages = packages.filter(pkg => pkg.services.includes("VR"));
  ...
  const selectedPackage = vrPackages.find(p => p.id === selectedPackageId);
  ...
  const renderPackagesWithForm = () => (
    <div className="space-y-6">
      <VRProposalPackagesTab
        vrPackages={vrPackages}
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3 text-sm text-muted-foreground">
            ...
            <div className="flex items-start space-x-2">
              ...
              />
              <Label htmlFor="vr-terms" className="text-sm leading-tight cursor-pointer">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="vrFirstName">First Name *</Label>
                <Input id="vrFirstName" value={formData.firstName} onChange={(e) => setFormData(f => ({ ...f, firstName: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="vrLastName">Last Name *</Label>
                <Input id="vrLastName" value={formData.lastName} onChange={(e) => setFormData(f => ({ ...f, lastName: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="vrEmail">Email Address *</Label>
                <Input id="vrEmail" type="email" value={formData.email} onChange={(e) => setFormData(f => ({ ...f, email: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="vrTelephone">Telephone *</Label>
                <Input id="vrTelephone" value={formData.telephone} onChange={(e) => setFormData(f => ({ ...f, telephone: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="vrCompanyName">Company Name</Label>
                <Input id="vrCompanyName" value={formData.companyName} onChange={(e) => setFormData(f => ({ ...f, companyName: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="vrHeardAboutUs">How did you hear about us? *</Label>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-between">
              <DialogTitle>Call Answering Service Proposal Builder</DialogTitle>
