...
import { DTProposalPackagesTab } from "./proposal/DTProposalPackagesTab";
...
interface DTPackageProposalDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialData?: ProposalInitialData;
  onPackageSelected?: (pkg: { name: string; digitalPricePerMinute: number }, proposal: ProposalRecord) => void;
}

export function DTPackageProposalDialog({ open, onOpenChange, initialData, onPackageSelected }: DTPackageProposalDialogProps) {
  const { packages } = usePackages();
  const dtPackages = packages.filter(pkg => pkg.services.includes("DT"));
  ...
  const selectedPackage = dtPackages.find(p => p.id === selectedPackageId);
  ...
  const handleFinalSubmit = () => {
    if (selectedPackage && onPackageSelected) {
      const proposalRecord: ProposalRecord = {
        ...
        packagePrice: selectedPackage.digitalPricePerMinute,
        invoiceNumber,
      ...
      onPackageSelected({
        ...
        digitalPricePerMinute: selectedPackage.digitalPricePerMinute,
      }, proposalRecord);
    }
    toast({
      title: "Proposal Submitted",
      description: `${formData.firstName} ${formData.lastName} selected the "${selectedPackage?.name}" DT package. Proposal, agreement and invoice have been sent.`,
    });
    onOpenChange(false);
    resetForm();
  };
  ...
  const renderPackagesWithForm = () => (
    <div className="space-y-6">
      <DTProposalPackagesTab
        dtPackages={dtPackages}
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3 text-sm text-muted-foreground">
            ...
            <div className="flex items-start space-x-2">
              ...
              />
              <Label htmlFor="dt-terms" className="text-sm leading-tight cursor-pointer">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="dtFirstName">First Name *</Label>
                <Input id="dtFirstName" value={formData.firstName} onChange={(e) => setFormData(f => ({ ...f, firstName: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="dtLastName">Last Name *</Label>
                <Input id="dtLastName" value={formData.lastName} onChange={(e) => setFormData(f => ({ ...f, lastName: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="dtEmail">Email Address *</Label>
                <Input id="dtEmail" type="email" value={formData.email} onChange={(e) => setFormData(f => ({ ...f, email: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="dtTelephone">Telephone *</Label>
                <Input id="dtTelephone" value={formData.telephone} onChange={(e) => setFormData(f => ({ ...f, telephone: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="dtCompanyName">Company Name</Label>
                <Input id="dtCompanyName" value={formData.companyName} onChange={(e) => setFormData(f => ({ ...f, companyName: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="dtHeardAboutUs">How did you hear about us? *</Label>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-between">
              <DialogTitle>Digital Typing Service Proposal Builder</DialogTitle>
