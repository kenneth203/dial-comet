...
import { CBProposalPackagesTab } from "./proposal/CBProposalPackagesTab";
...
interface CBPackageProposalDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialData?: ProposalInitialData;
  onPackageSelected?: (pkg: { name: string; price: number; minutes: number; overage: number }, proposal: ProposalRecord) => void;
}

export function CBPackageProposalDialog({ open, onOpenChange, initialData, onPackageSelected }: CBPackageProposalDialogProps) {
  const { packages } = usePackages();
  const cbPackages = packages.filter(pkg => pkg.services.includes("CB"));
  ...
  const selectedPackage = cbPackages.find(p => p.id === selectedPackageId);
  ...
  const handleFinalSubmit = () => {
    if (selectedPackage && onPackageSelected) {
      ...
      onPackageSelected({
        ...
        overage: selectedPackage.overage,
      }, proposalRecord);
    }
    toast({
  ...
  const renderPackagesWithForm = () => (
    <div className="space-y-6">
      <CBProposalPackagesTab
        cbPackages={cbPackages}
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/30 rounded-lg p-4 space-y-3 text-sm text-muted-foreground">
            ...
            <div className="flex items-start space-x-2">
              ...
              />
              <Label htmlFor="cb-terms" className="text-sm leading-tight cursor-pointer">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="cbFirstName">First Name *</Label>
                <Input id="cbFirstName" value={formData.firstName} onChange={(e) => setFormData(f => ({ ...f, firstName: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="cbLastName">Last Name *</Label>
                <Input id="cbLastName" value={formData.lastName} onChange={(e) => setFormData(f => ({ ...f, lastName: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="cbEmail">Email Address *</Label>
                <Input id="cbEmail" type="email" value={formData.email} onChange={(e) => setFormData(f => ({ ...f, email: e.target.value }))} required />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="cbTelephone">Telephone *</Label>
                <Input id="cbTelephone" value={formData.telephone} onChange={(e) => setFormData(f => ({ ...f, telephone: e.target.value }))} required />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="cbCompanyName">Company Name</Label>
                <Input id="cbCompanyName" value={formData.companyName} onChange={(e) => setFormData(f => ({ ...f, companyName: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="cbHeardAboutUs">How did you hear about us? *</Label>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-between">
              <DialogTitle>Call Answering Service (Bookings) Proposal Builder</DialogTitle>
